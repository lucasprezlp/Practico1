#include <iostream>

using namespace std;
/*Ejercicio 4: Escriba un programa que tome cada 4 horas la temperatura exterior,
 leyéndola durante un período de 24horas. Es decir, debe leer 6 temperaturas.

  Calcule la temperatura media del día, la temperatura más alta y la más baja.
*/

int main()
{


    float temperatura,suma_temperatura=0,temperatura_media,temperatura_alta=0;

    float temperatura_baja=9999;

    for(int i=0; i<24; i+=4)
    {

        cout<<"Ingrese la temperatura a las "<<i<<": " <<endl;
        cin>>temperatura;
        suma_temperatura += temperatura;
        if(temperatura > temperatura_alta)
        {
            temperatura_alta = temperatura;
        }
        if(temperatura < temperatura_baja)
        {
            temperatura_baja = temperatura;
        }
    }
    temperatura_media = suma_temperatura / 6;
    cout<<"La temperatura media del dia es: "<<temperatura_media<<endl;
    cout<<"La temperatura mas alta es: "<<temperatura_alta<<endl;
    cout<<"La temperatura mas baja es: "<<temperatura_baja<<endl;
    return 0;
}
